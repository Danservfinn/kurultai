...
if [ $ERRORS -eq 0 ] && [ $WARNINGS -eq 0 ]; then
    ...
    # Verify Kurultai effectiveness
    echo ""
    echo "Verifying Kurultai effectiveness..."
    python3 << 'PYEOF'
from neo4j import GraphDatabase
from datetime import datetime, timedelta
...
try:
    ...
    with driver.session() as session:
        # Check for recent hourly reflections
        result = session.run("""
            MATCH (t:TaskCompletion)
        ...
        if record and record['count'] >= 2:
            print(f"✅ Kurultai reflections: {record['count']} in last 2 hours - WORKING")
        elif record and record['count'] == 1:
            print(f"⚠️  Kurultai reflections: Only {record['count']} in last 2 hours")
        else:
            print(f"❌ Kurultai reflections: NONE in last 2 hours - ISSUE")
    
    driver.close()
except Exception as e:
    print(f"❌ Kurultai verification failed: {e}")
PYEOF
    ...
    echo ""
    echo "Critical issues found. Temüjin task will be created."
    HEALTH_STATUS="UNHEALTHY"
    
    # Create task for Temüjin
    TASK_FILE="/Users/kublai/.openclaw/agents/temujin/tasks/health-check-$(date +%Y%m%d-%H%M%S).md"
    cat > "$TASK_FILE" << EOF
# Urgent: Agent Harness Health Check Failed
...
Agent harness health check detected critical issues. Review health check logs for details.
...
2. Review output and identify root cause

3. Fix the issues

4. Re-run health check to verify fix

5. Update this task with resolution

---

**Auto-generated by:** Agent Harness Health Check
EOF
    
    echo "✅ Temüjin task created: $TASK_FILE"
    exit 1
fi
EOF
