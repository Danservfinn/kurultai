...
| MCP servers (Computer Use, Playwright) | [mcp-servers.md](mcp-servers.md), [task-executor.md](task-executor.md) |
...
- Files are derived from: `server.js`, `neo4j.js`, `kurultai.json`, `claude-agent`, `provider.env`, `mode.json`, `settings.json`, `task_executor.py`, `neo4j_v2_events.py`, `asmr_extractor.py`, `context_profile.py`, `supersede_detector.py`, `asmr_schema_validator.py`, `parallel_memory_search.py`, `computer_use_mcp.py`, `computer-use-mcp.json`, `computer-use-agents.txt`, and agent `CLAUDE.md` files
