...
Usage:
    from completion_gate_audit_v2 import completion_gate_audit_v2, AuditResultV2
...
# Load valid agents
VALID_AGENTS_FILE = Path(AGENTS_DIR) / "main" / "config" / "valid-agents.json"


def load_valid_agents() -> set:
    """Load valid agent names from config file."""
    try:
        with open(VALID_AGENTS_FILE, 'r') as f:
            data = json.load(f)
            return set(data.get("agents", []))
    except Exception:
        # Fallback to hardcoded list
        return {"mongke", "chagatai", "temujin", "jochi", "ogedei", "kublai", "tolui"}


VALID_AGENTS = load_valid_agents()
    ...
    if total_calls > 0 and total_calls >= 10:
        ...
        if failure_rate >= CIRCUIT_BREAKER_THRESHOLD:
            ...
            if not state.is_open:
                state.is_open = True
                state.llm_disabled_until = (datetime.now() + timedelta(minutes=CIRCUIT_BREAKER_COOLDOWN_MINUTES)).isoformat()
...
{{
  ...
  "can_complete": false,
  "missing_components": ["Credit pack display not fixed"],
...
def permissive_pass_through(
    ...
    Returns conservative 50% completion with warning.
    """
    ...
    append_ledger({
        ...
        "warning": "All audit tiers failed - using permissive pass-through"
    })

    return {
        "completion_percentage": 50,  # Conservative
        "can_complete": True,  # Allow completion with warning
        "missing_components": [],
        "quality_issues": ["Audit system unavailable - manual review recommended"],
        "required_followups": [],
        "optional_improvements": [
            {
                ...
                "agent": default_agent,
                "priority": "high",
                "reason": "Completion gate audit system was unavailable"
            }
        ],
        "blockers": []
    }
