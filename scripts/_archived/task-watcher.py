    ...
    try:
        with driver.session() as session:
            result = session.run("""
                ...
                WHERE coalesce(t.v2_eligible, false) = false
...
def _already_completed(executing_file):
    ...
    for done_file in parent.iterdir():
        if not done_file.name.endswith('.done.md'):
            continue
        # Check for all terminal states - patterns must be checked in order
        # .credential_failed.done.md added 2026-03-09 for auth failures
        # .investigation_required.done.md added 2026-03-11 for O002 fast failures
        done_stem = None
        ...
        elif '.credential_failed.done.md' in done_file.name:
            done_stem = done_file.name.split('.credential_failed.done.md')[0]
        elif '.investigation_required.done.md' in done_file.name:
            done_stem = done_file.name.split('.investigation_required.done.md')[0]
    ...
    for agent_dir in AGENTS_DIR.iterdir():
        ...
        for f in tasks_dir.iterdir():
            ...
            if f.is_file() and f.name.endswith('.executing.md'):
                # Skip completed task patterns (including O002 terminal states)
                if any(f.name.endswith(pattern) for pattern in [
                    ...
                    ".credential_failed.done.md.executing.md",
                    ".investigation_required.done.md.executing.md",
