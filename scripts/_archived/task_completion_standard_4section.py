...
def score_report_quality(content: str, metadata: Optional[Dict] = None) -> Dict:
    ...
    # Return dict format expected by completion-audit.py and task-report-hook.py
    return {
        "overall_score": total,
        "breakdown": breakdown,
        "recommendations": breakdown["recommendations"],
        "missing_sections": breakdown["missing_sections"],
        # Additional fields expected by task-report-hook.py
        "has_summary": bool(re.search(r'^##\s*Problem\b', content, re.MULTILINE | re.IGNORECASE)),
        "has_changes": bool(re.search(r'(modified|created|changed|updated)', content, re.IGNORECASE)),
        "has_verification": bool(extract_section_content(content, "verification")[0])
    }


def extract_completion_summary(content: str, max_length: int = 200) -> str:
    ...
    4. Falls back to Solution section content

    Args:
        content: The full task output
    ...
    # Fallback: extract from Problem or Solution section
    _, problem_content = extract_section_content(content, "problem")
    if problem_content:
        return problem_content.split('\n')[0][:max_length]

    _, solution_content = extract_section_content(content, "solution")
    if solution_content:
        return solution_content.split('\n')[0][:max_length]
...
"""
    ...
    result = score_report_quality(sample_good, {})
    print(f"  Score: {result['overall_score']}")

    print("\nTesting bad report:")
    valid, missing, rtype = validate_completion_report(sample_bad, {})
    print(f"  Valid: {valid}, Missing: {missing}, Type: {rtype}")
    result = score_report_quality(sample_bad, {})
    print(f"  Score: {result['overall_score']}")
