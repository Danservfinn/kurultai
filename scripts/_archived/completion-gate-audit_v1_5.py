...
# Import autoresearch completion gate integration
try:
    from autoresearch_completion_gate import check_autoresearch_task_completion
    AUTORESEARCH_GATE_AVAILABLE = True
except ImportError:
    AUTORESEARCH_GATE_AVAILABLE = False
    print("[WARN] autoresearch_completion_gate module not available, autoresearch checks disabled")

# Gate audit log directory (imported from gate_utils)
...
- Require a "## Resolution" section for all substantive completions (>=100 chars output)
...
6. RESOLUTION_SECTION: REQUIRED for all substantive completions (>=100 chars). Must include "## Resolution" heading with "What Was Done" subsection.
...
{{
  ...
  "quality_issues": ["issue1", "issue2", "Missing ## Resolution section"],
  "required_followups": [
...
NOTE: If execution output >= 100 chars but lacks "## Resolution" heading, add to quality_issues and set can_complete=false.
...
# LLM audit cache and timeout configuration
_LLM_AUDIT_CACHE: dict = {}  # Simple in-memory cache
_LLM_AUDIT_CACHE_MAX_SIZE = 100  # Maximum cache entries

# Import gate timeout configuration
try:
    from gate_timeouts import GATE_AUDIT_TIMEOUT_SECONDS
    LLM_AUDIT_TIMEOUT_SECONDS = GATE_AUDIT_TIMEOUT_SECONDS
except ImportError:
    # Fallback if gate_timeouts not available
    LLM_AUDIT_TIMEOUT_SECONDS = 300  # 5 minutes (task requirement)


def _get_cache_key(audit_prompt: str) -> str:
    """Generate cache key from prompt content."""
    import hashlib
    # Use first 500 chars of prompt + hash for cache key
    prompt_hash = hashlib.sha256(audit_prompt.encode()).hexdigest()[:16]
    return f"audit_{prompt_hash}"


def call_llm_audit(audit_prompt: str) -> Optional[dict]:
    """Call Claude Opus 4.6 for audit analysis.

    Uses the Anthropic SDK to perform intelligent completion audits.
    Falls back to template-based audit on any failure.

    Features:
    - Caching: Results cached for 5 minutes to avoid duplicate calls
    - Timeout: 30 second maximum for API calls
    - Rate limit handling: Graceful fallback on rate limits

    Args:
        audit_prompt: Structured prompt with boundaries (from build_sanitized_prompt)

    Returns:
        Parsed JSON audit result, or None if failed (triggers fallback)
    """
    import time

    # Check cache first
    cache_key = _get_cache_key(audit_prompt)
    if cache_key in _LLM_AUDIT_CACHE:
        cached_entry = _LLM_AUDIT_CACHE[cache_key]
        # Cache entries expire after 5 minutes
        if time.time() - cached_entry["timestamp"] < 300:
            print(f"[INFO] LLM audit cache hit for {cache_key}")
            return cached_entry["result"]
        else:
            # Remove expired entry
            del _LLM_AUDIT_CACHE[cache_key]

    try:
        import httpx
        from anthropic import Anthropic

        # Read credentials from environment (supports both ANTHROPIC_API_KEY and ANTHROPIC_AUTH_TOKEN)
        api_key = os.environ.get('ANTHROPIC_API_KEY') or os.environ.get('ANTHROPIC_AUTH_TOKEN')
        base_url = os.environ.get('ANTHROPIC_BASE_URL')

        if not api_key:
            print("[WARN] LLM audit skipped: No API key found (ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN)")
            return None

        # Initialize client with explicit credentials
        client_kwargs = {
            'timeout': httpx.Timeout(LLM_AUDIT_TIMEOUT_SECONDS, connect=5.0)
        }

        # Only set api_key if it's a valid Anthropic key (not a proxy token format)
        # Proxy tokens (like Z.AI's format) need the base_url to work properly
        if api_key.startswith('sk-ant-'):
            client_kwargs['api_key'] = api_key
        elif base_url:
            # For proxy tokens, pass via env and set base_url
            client_kwargs['base_url'] = base_url
            client_kwargs['api_key'] = api_key
        else:
            print(f"[WARN] LLM audit skipped: Proxy token format detected but no BASE_URL set")
            return None
        ...
        if not response.content or len(response.content) == 0:
            print("[WARN] LLM audit returned empty response")
            return None
        ...
        if not isinstance(audit_data, dict):
            print("[WARN] LLM audit returned non-dict result")
            return None

        # Log successful LLM audit
        print(f"[INFO] LLM audit completed using claude-opus-4-6")
        print(f"[INFO] Model reported completion: {audit_data.get('completion_percentage', 'N/A')}%")

        # Cache the result
        if len(_LLM_AUDIT_CACHE) >= _LLM_AUDIT_CACHE_MAX_SIZE:
            # Remove oldest entry
            oldest_key = min(_LLM_AUDIT_CACHE.keys(),
                           key=lambda k: _LLM_AUDIT_CACHE[k]["timestamp"])
            del _LLM_AUDIT_CACHE[oldest_key]
        ...
        print("[WARN] anthropic SDK not installed - falling back to template audit")
        return None
    except Exception as e:
        ...
        if "api_key" in str(e).lower() or "authentication" in str(e).lower():
            print(f"[WARN] LLM audit skipped: API key not configured")
        elif "rate" in str(e).lower() or "limit" in str(e).lower():
            print(f"[WARN] LLM audit skipped: Rate limited")
        elif "timeout" in str(e).lower():
            print(f"[WARN] LLM audit skipped: Request timed out")
        else:
            print(f"[WARN] LLM audit failed ({error_type}): {error_msg}")

        return None  # Triggers fallback to template audit
...
def fallback_template_audit(task_content: dict, execution_output: str, task_body: str) -> dict:
    ...
    # Pattern 8: Missing resolution section (required for substantive completions)
    # Matches quality_gate.py detection logic
    if len(safe_output.strip()) >= 100:  # Only required for substantive content
        has_resolution = (
            "## Resolution" in safe_output
            or "**Status:**" in safe_output
            or "## Result" in safe_output
            or "## Summary" in safe_output
        )
        if not has_resolution:
            missing.append("Resolution section missing - add ## Resolution or ## Summary section")
...
def completion_gate_audit(task_file: Path, agent: str) -> AuditResult:
    ...
    # Step 5.6: Autoresearch completion gate check
    # Block autonomous research tasks from completing without approval
    if AUTORESEARCH_GATE_AVAILABLE:
        can_complete, ar_reason, ar_request_id = check_autoresearch_task_completion(
            task_id=task_id,
            agent=agent,
            execution_output=execution_output
        )
        if not can_complete:
            print(f"[AUTORESEARCH-GATE] Blocking completion: {ar_reason}")
            audit_data["can_complete"] = False
            audit_data["completion_percentage"] = min(audit_data.get("completion_percentage", 0), 50)
            audit_data["blockers"] = audit_data.get("blockers", []) + [ar_reason]

            # Add follow-up for approval
            if ar_request_id:
                audit_data["required_followups"] = audit_data.get("required_followups", []) + [{
                    "title": f"Obtain Kublai approval for autoresearch (request: {ar_request_id})",
                    "agent": "kublai",
                    "priority": "high",
                    "reason": ar_reason
                }]
            ...
            if SECURITY_LOGGER_AVAILABLE:
                log_security_event(
                    event_type=EventType.TASK_BLOCKED if EventType else None,
                    severity=Severity.MEDIUM if Severity else None,
                    details={
                        "task_id": task_id,
                        "agent": agent,
                        "reason": "autoresearch_gate",
                        "blocker": ar_reason
                    }
                )
