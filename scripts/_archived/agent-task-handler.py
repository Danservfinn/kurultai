...
# Fallback is now handled by the claude-agent wrapper (primary/backup settings.json)
TIMEOUT_BY_PRIORITY = {
...
def categorize_error(error_msg: str, output_content: str = None) -> str:
    ...
    Returns:
        Error type string: SIGKILL, SIGTERM_KILLED, PROXY_AUTH, PROXY_ERROR, RATE_LIMIT,
                          MODEL_FAILURE, STALL_TIMEOUT, VERIFICATION_FAILED, AUTH, or UNKNOWN
    """
    if not error_msg:
        return "UNKNOWN"
    ...
    if any(pattern in combined_lower for pattern in [
        "signal 9", "signal -9", "sigkill", "killed by signal",
        "exit_code=-9", "exit_code=9", "returncode=-9", "returncode=9",
    ]):
        return "SIGKILL"  # Hard kill (OOM, system kill, or timeout)
...
# FIX (2026-03-12 ogedei): 60s was too short — agents need time to load context,
# read rules.json (M003), output RULES LOADED, then invoke skill. 60s caused
# fleet-wide R008_VIOLATION cascade (29 failures in 2h, 100% failure rate).
R008_PREFLIGHT_ELAPSED = 180   # check for skill invocation after 180 seconds


def _skill_exists(skill_hint: str, agent_name: str = None) -> bool:
    """Check if a skill exists in OpenClaw.

    Skills can be:
    - Global: ~/.openclaw/skills/<skill_name>/SKILL.md
    - Agent-specific: ~/.openclaw/agents/<agent>/skills/<skill_name>/SKILL.md

    Args:
        skill_hint: Skill hint (e.g., '/kurultai-health')
        agent_name: Optional agent name to check agent-specific skills
    ...
    # Strip leading '/' from skill hint
    skill_name = skill_hint.lstrip('/')
    if not skill_name:
        return False

    # Check global skills directory
    global_skill_path = os.path.expanduser(f"~/.openclaw/skills/{skill_name}/SKILL.md")
    if os.path.exists(global_skill_path):
        return True

    # Check agent-specific skills directory if agent provided
    if agent_name:
        agent_skill_path = os.path.expanduser(f"~/.openclaw/agents/{agent_name}/skills/{skill_name}/SKILL.md")
...
def load_agent_config(agent_name):
    """Load agent configuration from kurultai.json (single source of truth)."""
...
def _run_pre_submit_check(task_file):
    ...
    M001 FIX (2026-03-12): Removed permissive fallbacks that allowed tasks to pass
    without quality verification. Now BLOCKS completion when checks fail.

    The check validates:
    - Content length (200+ chars, lowered from 500 2026-03-11)
    ...
    if not os.path.exists(script_path):
        print(f"[PRE_SUBMIT] ❌ BLOCKED: pre_submit_check.py not found at {script_path}")
        return False, f"Quality gate script missing: {script_path}"
        ...
        if result.returncode == 0:
            ...
            if result.stdout:
                for line in result.stdout.split('\n'):
                    if 'Content length' in line or 'Structure' in line or 'Resolution' in line or 'Skill invocation' in line:
                        reason = line.strip()
                        break
            print(f"[PRE_SUBMIT] ❌ BLOCKED: {reason}")
            return False, reason
    except subprocess.TimeoutExpired:
        # Timeout is transient — allow completion but log for monitoring
        print(f"[PRE_SUBMIT] ⚠ Check timed out (allowing completion - transient)")
        return True, ""
    except Exception as e:
        # Non-timeout errors indicate real problems — block completion
        print(f"[PRE_SUBMIT] ❌ BLOCKED: Check error: {e}")
...
def mark_task_completed(task_file, status='completed', executing_file=None):
    ...
    if os.path.exists(executing_file):
        ...
        if success:
            ...
            # LOG COMPLETED EVENT (2026-03-12 fix for false 100% failure rate)
            # Tasks were marked .done.md but COMPLETED events were never logged.
            # This caused ledger to show only FAILED events, triggering false HIGH_FAILURE_RATE alerts.
            # Log COMPLETED for all terminal states (completed, failed, no_output, etc.)
            try:
                task_id = None
                try:
                    with open(completed_file, 'r', encoding='utf-8', errors='ignore') as f:
                        content = f.read(3000)
                        task_match = re.search(r'^task_id:\s*(\S+)', content, re.MULTILINE)
                        if task_match:
                            task_id = task_match.group(1)
                except Exception:
                    pass

                # Determine success flag for the event
                is_success = (status == 'completed')

                # Count output lines (proxy for work done)
                output_lines = 0
                try:
                    with open(completed_file, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                _append_ledger({
                    ...
                    "agent": agent,
                    "status": status,  # completed, failed, no_output, etc.
                    "success": is_success,
                    "output_lines": output_lines,
                })
                print(f"[LEDGER] Logged COMPLETED event for {task_id or 'unknown'} (status={status})")
            except Exception as e:
                print(f"[LEDGER] Failed to log COMPLETED event (non-fatal): {e}")
    ...
    for candidate in sorted(glob.glob(os.path.join(task_dir, f"{stem}*.md"))):
        ...
        if success:
            ...
            # LOG COMPLETED EVENT (2026-03-12 fix for false 100% failure rate)
            # Fallback path also needs to log COMPLETED events
            try:
                task_id = None
                try:
                    with open(completed_file, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                print(f"[LEDGER] Logged COMPLETED event for {task_id or 'unknown'} (fallback, status={status})")
            except Exception as e:
                print(f"[LEDGER] Failed to log COMPLETED event in fallback (non-fatal): {e}")
...
def _check_skill_invocation_early(agent_name: str, start_time: float, skill_hint: str) -> bool:
    ...
    R008 FIX (2026-03-12): Now checks for skill APPLICATION, not just invocation method.
    - Method 1: Explicit Skill tool call (original behavior)
    - Method 2: Skill protocol evidence in assistant output (new - handles auto-injected skills)

    Returns True if skill_hint was found in session or skill protocol is being followed, False otherwise.
    """
    if not skill_hint:
    ...
    # NOTE: Changed from `mt >= start_time` to avoid mtime race condition.
    # Session files may be created before start_time, and mtime may not update
    # immediately when Skill tool is invoked. Instead, find most recent file
    # and verify it has activity since start_time by checking content.
    newest = None
    newest_mtime = 0.0
    for jf in project_dir.glob("*.jsonl"):
        try:
            mt = jf.stat().st_mtime
            # Look for files modified within 120 seconds window around start_time
            # This accounts for files created before task start but written to during
            if mt > start_time - 120 and mt > newest_mtime:
                newest_mtime = mt
                newest = jf
        except OSError:
            continue
    ...
    # Verify file has activity since start_time (content-based check)
    # This prevents checking old session files that happen to have recent mtime
    ...
    for line in lines:
        try:
            ...
            if ts_str:
                # Parse ISO timestamp and check if it's after start_time
                # start_time is Unix timestamp, ts_str is ISO format
                from dateutil import parser as _date_parser
                try:
                    ts_dt = _date_parser.parse(ts_str)
                    ts_unix = ts_dt.timestamp()
                    if ts_unix >= start_time:
    ...
    if not has_recent_activity:
        # File exists but has no activity since start_time - skip it
        return False
    ...
    # Method 2: R008 FIX - Check for skill protocol evidence in assistant output
    # This handles auto-injected skills where agent follows protocol without explicit tool call
    # Collect all assistant message text since start_time
    output_text = []
    for line in lines:
        try:
            record = json.loads(line)
    ...
    combined_output = " ".join(output_text).lower()

    # Skill evidence patterns (same as _verify_task_completion)
    skill_evidence = {
        '/horde-learn': ['search', 'source', 'research', 'citation', 'finding', 'web'],
        '/horde-brainstorming': ['proposal', 'brainstorm', 'option', 'approach', 'explore'],
        '/horde-implement': ['implement', 'code', 'file', 'write', 'create'],
    ...
    # Check for evidence of skill usage in output
    has_evidence = False
    for keyword in skill_evidence.get(skill_hint, [hint_normalized.replace('-', ' ')]):
        if keyword in combined_output:
            has_evidence = True
            break

    # Also check for explicit skill invocation patterns in text
    skill_patterns = [
        skill_hint.lower(),
        'skill invoked',
        'using skill',
    ...
    if any(p in combined_output for p in skill_patterns):
        has_evidence = True

    if has_evidence:
        # Log that skill protocol was detected (auto-injected case)
        _append_ledger({
            ...
            "detection_method": "output_evidence"
        })
        return True  # Skill protocol detected in output
...
def auth_health_preflight(agent: str, timeout: int = 30) -> bool:
    ...
    """
    # First check cached auth status (15-minute stale threshold)
    # FIX (2026-03-12): Increased from 5min to 15min to reduce concurrent auth checks
    # that cause fleet-wide timeouts during parallel task execution.
    heartbeat_file = Path(AGENTS_DIR) / "main" / "logs" / "auth-heartbeat.json"
    AUTH_STALE_SECONDS = 900  # 15 minutes - auth_heartbeat runs every 5min
...
def _call_claude_code(agent_name, prompt, config, skill_hint=None, timeout=None, model=None, priority='normal'):
    ...
    from kurultai_paths import VALID_CLAUDE_MODELS
    ...
    try:
        ...
        while True:
            ...
            # EMERGENCY FIX (2026-03-12 15:57): Skip R008 check if skill doesn't exist in OpenClaw
            # This prevents fleet-wide stalls when tasks reference non-existent skills like /kurultai-health
            if skill_hint and elapsed >= R008_PREFLIGHT_ELAPSED:
                if not _skill_exists(skill_hint, agent_name):
                    # Skill doesn't exist in OpenClaw - skip R008 enforcement
                    # This allows tasks with Claude Code "horde skills" or other non-OpenClaw skills to proceed
                    print(f"  ℹ️  R008_SKIPPED: Skill '{skill_hint}' not found in OpenClaw, skipping enforcement")
                    _append_ledger({
                        "event": "R008_SKILL_NOT_FOUND",
                        "ts": datetime.now().isoformat(),
                        "agent": agent_name,
                        "skill_hint": skill_hint,
                ...
                elif not _check_skill_invocation_early(agent_name, start, skill_hint):
...
def execute_task_with_llm(agent_name, task_text, config, skill_hint=None, timeout=None, model=None, priority='normal', task_id=None, context_history=None, audience=None):
    ...
    Rate limit fallback is handled by the claude-agent wrapper (primary/backup settings.json).
    ...
    if active_rules:
        rules_first_section = "\n\n## MANDATORY FIRST ACTION (M003 Rules Self-Check)\nBefore ANY other work, read ~/.openclaw/agents/{}/rules.json and output 'RULES LOADED: [rule IDs]' at the START of your response. This is required for every task.\n\n".format(agent_name)
...
"""
    ...
    # SESSION HEALTH PREFLIGHT: Check for session bloat before executing task (2026-03-12)
    # Prevents SIGKILL failures from memory pressure when loading bloated sessions
    # Pattern from session_health_watchdog.py - main agent bloat causes child process SIGKILLs
    try:
        import session_health_watchdog
        # Check main agent sessions (the spawner) - its bloat causes child SIGKILLs
        main_healthy, main_reason = session_health_watchdog.check_agent_health("main")
        if not main_healthy:
            print(f"  SESSION PREFLIGHT: main agent unhealthy ({main_reason}) — running cleanup")
            session_health_watchdog.cleanup_agent_sessions("main", dry_run=False)

        # Check target agent sessions
        target_healthy, target_reason = session_health_watchdog.check_agent_health(agent_name)
        if not target_healthy:
            print(f"  SESSION PREFLIGHT: {agent_name} unhealthy ({target_reason}) — running cleanup")
            session_health_watchdog.cleanup_agent_sessions(agent_name, dry_run=False)
    except Exception as e:
        print(f"  ⚠ Session health preflight failed (non-blocking): {e}")

    # Execute task — fallback is handled by the claude-agent wrapper (primary/backup settings.json)
...
def _validate_domain_compliance(agent_name, task_content, task_id):
    ...
    # Exclude skill references (e.g., /horde-implement) from domain validation
    # These are execution instructions, not task content
    import re
    task_content_no_skills = re.sub(r'/[\w-]+', '', task_content)
    task_lower = task_content_no_skills.lower()
...
def _cleanup_orphan_processes(agent_name: str, older_minutes: int = 60) -> None:
    """Kill stale Claude processes before task execution to prevent OOM-induced SIGKILL.

    PRIORITY_FIX (horde-review 2026-03-12): zombie Claude processes accumulate between
    tock runs and consume RAM, causing new task processes to receive SIGKILL (-9) at ~70s.
    Running this before each dispatch clears the backlog so the task gets full memory budget.
    """
    cleanup_script = Path(__file__).parent / "cleanup-orphan-claude.sh"
    if not cleanup_script.exists():
        return
    try:
        result = subprocess.run(
            ["/bin/bash", str(cleanup_script), "--run", f"--older-minutes={older_minutes}"],
            capture_output=True, text=True, timeout=30
        )
        output = (result.stdout or "").strip()
        if output:
            for line in output.split('\n'):
                if any(kw in line for kw in ('Terminat', 'orphan', 'Found', 'Cleanup complete')):
                    print(f"  🧹 [orphan-cleanup] {line.strip()}")
    except Exception as e:
        print(f"  ⚠ Orphan cleanup failed (non-blocking): {e}")
...
def process_task(agent_name, task):
    ...
    # Strip CLAUDE_CODE_PREAMBLE before validation to avoid false positives
    # (e.g., "/horde-implement" in example skills triggering "implement" keyword)
    validation_content = task_content
    if "**EXECUTION METHOD:**" in task_content:
        # Find the end of the preamble (double newline after instructions)
        preamble_end = task_content.find("\n\n", task_content.find("**EXECUTION METHOD:**"))
        if preamble_end > 0:
            validation_content = task_content[preamble_end + 2:].lstrip()

    domain_fail = _validate_domain_compliance(agent_name, validation_content, task_id)
    if domain_fail:
    ...
    # Pre-execution orphan cleanup: free RAM before spawning new Claude process.
    # Prevents OOM-induced SIGKILL (-9) from zombie processes accumulating between tock runs.
    _cleanup_orphan_processes(agent_name)

    # Initialize start_time before conditional blocks — used for elapsed time calculation
    # and for extracting skill invocations from session transcript after execution
    start_time = time.time()
    ...
    if result.get('success'):
        ...
        gc.collect()  # Release memory after successful task
        return True
    else:
        ...
        gc.collect()  # Release memory after failed task
        return False
