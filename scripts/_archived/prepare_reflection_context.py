...
# Compile frequently-used regex patterns once (module-level cache)
# This reduces CPU overhead during hourly reflection cycles (6 agents × 1 call/hour)
_REGEX_CACHE = {
    "date_pattern": re.compile(r"\d{4}-\d{2}-\d{2}\.md"),
    "active_rules_section": re.compile(
        r"#{1,4} (?:ACTIVE |Active |YOUR BEHAVIORAL |WHEN/THEN )?RULES.*?\n(.*?)(?=\n#{1,4} |\Z)",
        re.DOTALL | re.IGNORECASE,
    ),
    "when_keyword": re.compile(r"WHEN\b", re.IGNORECASE),
    "numbering_prefix": re.compile(r"^\d+[\.\)]\s*"),
    "bold_parens_prefix": re.compile(r"^\*{2}\([^)]+\)\*{2}\s*"),
    "list_marker": re.compile(r"^-\s*"),
    # Template placeholder pattern — used in 3+ places, compile once
    "template_placeholder": re.compile(
        r"\[trigger\]|\[action\]|\[old default\]|\[default\]|\[new WHEN/THEN rule\]|\[none\]|RULE: \[new",
        re.IGNORECASE,
    ),
    "when_then_pattern": re.compile(r"NEW RULE|Rule \d+|WHEN\b", re.IGNORECASE),
    "rule_continuation": re.compile(r"(?:\n\n|\n\d+[\.\)]|$|\n\*\*|\n#)"),
    "when_then_line": re.compile(r"\bWHEN\b.+\bTHEN\b", re.IGNORECASE),
    "bold_markdown": re.compile(r"^\*{2,}[^*]+\*{2,}[:\s]*"),
    "commitment_block": re.compile(r"\*\*NEW RULE:\*\*\s*(.+?)(?=\n\*\*|\n##|\Z)", re.DOTALL),
    "commitment_fallback": re.compile(
        r"(?:COMMITMENT|Decision|Next|Plan):\s*(.+?)(?=\n\*\*|\n##|\Z)",
        re.IGNORECASE | re.DOTALL,
    ),
    "i_commit_pattern": re.compile(r"(?:I will|I commit to)\s+(.+?)(?:\.|$)", re.IGNORECASE),
    "h1_title": re.compile(r"^# (.+)", re.MULTILINE),
    "domain_extract": re.compile(r"\*\*Domain:\*\*\s*(.+)"),
    "category_extract": re.compile(r"\*\*Category:\*\*\s*(\w+)"),
    "status_section": re.compile(r"## Status\s*\n(.*?)(?=## |$)", re.DOTALL),
    "implemented_flag": re.compile(r"\*\*Implemented:\*\*\s*(YES|NO|PARTIAL)"),
    "resolution_section": re.compile(r"## Resolution\s*\n(.*?)(?=## |$)", re.DOTALL),
    "problem_section": re.compile(r"## Problem\s*\n(.*?)(?=## |$)", re.DOTALL),
}
...
def find_latest_memory_file(agent):
    ...
    for f in files:
        if _REGEX_CACHE["date_pattern"].match(f.name):
            return f
...
def extract_active_rules(memory_file):
    ...
    active_section = _REGEX_CACHE["active_rules_section"].search(content)
    if active_section:
        for line in active_section.group(1).strip().split("\n"):
            ...
            if not line or line.startswith("#"):
                continue
            if line and _REGEX_CACHE["when_keyword"].search(line):
                # Clean up numbering and bold markers
                cleaned = _REGEX_CACHE["numbering_prefix"].sub("", line)
                cleaned = _REGEX_CACHE["bold_parens_prefix"].sub("", cleaned)
                cleaned = _REGEX_CACHE["list_marker"].sub("", cleaned)
                cleaned = cleaned.strip()
                # Skip template placeholders
                if len(cleaned) > 10 and not _REGEX_CACHE["template_placeholder"].search(cleaned):
                    rules.append(cleaned)

    # Scan for WHEN/THEN patterns after NEW RULE headers (standard reflection format)
    if not rules:
        for match in _REGEX_CACHE["when_then_pattern"].finditer(content):
            start = match.end()
            # Find the end of the rule (next blank line, numbering, bold, or heading)
            end_match = _REGEX_CACHE["rule_continuation"].search(content, start)
            if end_match:
                rule = content[start:end_match.start()].strip()
            else:
                # If no end marker found, take next 200 chars
                rule = content[start:start + 200].strip()

            # Skip template placeholders
            if len(rule) > 10 and not _REGEX_CACHE["template_placeholder"].search(rule):
                rules.append(rule)
    ...
    if not rules:
        for line in content.split("\n"):
            ...
            if not line or line.startswith("#"):
                continue
            if _REGEX_CACHE["when_then_line"].search(line):
                # Clean markdown formatting (order matters: list markers first, then bold)
                cleaned = _REGEX_CACHE["list_marker"].sub("", line)
                cleaned = _REGEX_CACHE["numbering_prefix"].sub("", cleaned)
                cleaned = _REGEX_CACHE["bold_markdown"].sub("", cleaned)
                cleaned = cleaned.strip()
...
def extract_last_commitment(memory_file):
    ...
    commitments = _REGEX_CACHE["commitment_block"].findall(content)
    if commitments:
        # Filter out template placeholders
        real = [c.strip()[:200] for c in commitments
                if not _REGEX_CACHE["template_placeholder"].search(c)]
        if real:
            return real[-1]

    # Fallback: look for Decision/Commitment/Plan patterns
    matches = _REGEX_CACHE["commitment_fallback"].findall(content)
    if matches:
        return matches[-1].strip()[:200]

    matches = _REGEX_CACHE["i_commit_pattern"].findall(content)
    if matches:
        return matches[-1].strip()[:200]
...
def get_proposal_outcomes_block(agent, limit=3):
    """Extract recent implemented proposals for an agent.

    Returns a markdown block showing the agent's recent brainstorm
    proposals that were marked "Implemented: YES" with their outcomes.

    This closes the loop between brainstorming and reflection — agents
    see their ideas turn into actions.
    """
    proposals_dir = BASE / "main" / "proposals"
    if not proposals_dir.exists():
        return None

    try:
        # Find this agent's implemented proposals
        agent_proposals = []
        for filepath in sorted(proposals_dir.glob(f"{agent}-*.md"), reverse=True):
            try:
                content = filepath.read_text(encoding="utf-8", errors="replace")

                # Extract status section
                status_match = _REGEX_CACHE["status_section"].search(content)
                if not status_match:
                    continue

                status_text = status_match.group(1)
                impl_match = _REGEX_CACHE["implemented_flag"].search(status_text)

                # Only include implemented proposals
                if impl_match and impl_match.group(1) == "YES":
                    # Extract key fields
                    title_match = _REGEX_CACHE["h1_title"].search(content)
                    title = title_match.group(1) if title_match else filepath.name

                    domain_match = _REGEX_CACHE["domain_extract"].search(content)
                    domain = domain_match.group(1).strip() if domain_match else "general"

                    category_match = _REGEX_CACHE["category_extract"].search(status_text)
                    category = category_match.group(1) if category_match else "unknown"

                    # Extract Resolution if available (what was actually done)
                    resolution_match = _REGEX_CACHE["resolution_section"].search(content)
                    resolution = resolution_match.group(1).strip()[:200] if resolution_match else ""

                    # Extract Problem for context
                    problem_match = _REGEX_CACHE["problem_section"].search(content)
                    problem = problem_match.group(1).strip()[:150] if problem_match else ""
        ...
        # Format output block
        lines = ["## Previous Brainstorm Proposals & Outcomes"]
        for p in agent_proposals:
            lines.append(f"- **{p['title']}** ({p['domain']}, {p['category']})")
            if p['problem']:
...
def generate_context(agent, hours=1):
    ...
    # Proposal Outcomes — closes the loop on brainstorm proposals
    proposal_outcomes_block = get_proposal_outcomes_block(agent, limit=3)
    if proposal_outcomes_block:
        lines.append(proposal_outcomes_block)
        _sources["proposal_outcomes"] = "ok"
    else:
        _sources["proposal_outcomes"] = "empty"
