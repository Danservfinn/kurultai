...
def fallback_template_audit(task_content: Dict[str, Any], execution_output: str) -> Dict[str, Any]:
    ...
    if 'endpoint' in execution_output.lower() or 'api' in execution_output.lower():
        if 'error' not in execution_output.lower() and 'catch' not in execution_output.lower():
            quality_issues.append("Add error handling")
    ...
    if 'implement' in task_title.lower() or 'add' in task_title.lower():
        if 'doc' not in execution_output.lower():
            quality_issues.append("Add documentation")
    ...
    # Pattern 6: Missing resolution section (required for substantive completions)
    # This matches the requirement in task-completion-template.md
    # CRITICAL: Resolution section is now a BLOCKER (missing_components not quality_issues)
    # because /horde-review found 9/10 reports missing this section
    if len(execution_output.strip()) >= 100:
        ...
        if not has_resolution:
            # Changed from quality_issues to missing - this is now a gate blocker
            missing.append("Missing ## Resolution section - add ## Resolution heading with What Was Done subsection")

    can_complete = len(missing) == 0 and len(quality_issues) == 0
    completion_percentage = 100 if can_complete else max(50, 100 - len(missing) * 15 - len(quality_issues) * 5)
...
{{
  ...
  "can_complete": false,
  "missing_components": ["Credit pack display not fixed", "Webhook handler missing", "Missing ## Resolution section"],
