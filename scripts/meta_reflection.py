...
Two modes:
  --protocol: Option B compact reflections (~800 tokens) with role-specific
              protocols, WHEN/THEN behavioral rules, and commitment tracking.
  (default):  Legacy mode with generic 5-question template (~6400 tokens).

Usage:
    python3 meta_reflection.py --agent temujin --hours 1 --protocol
    python3 meta_reflection.py --all-agents --hours 1 --protocol
    python3 meta_reflection.py --agent temujin --hours 1  # legacy mode
"""
...
def generate_reflection(agent, hours=1, include_chat_review=False, chat_hours=2, include_heartbeat_review=False):
    """Generate meta-reflection prompt for an agent (legacy mode)"""
    metrics = get_agent_metrics(agent, hours)
...
def main():
    ...
    parser.add_argument('--protocol', action='store_true',
                        help='Use Option B protocol-based reflections (~800 tokens vs ~6400)')
