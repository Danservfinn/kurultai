...
### Issue: STUCK TASK RECOVERY Tasks (Self-Wake Deadlock)

**Meaning:** When an agent has pending tasks but no active session for 30+ minutes, `agent-self-wake.py` creates a "STUCK TASK RECOVERY" task. If these tasks themselves don't execute, the system enters a deadlock where agents have recovery tasks they can't process.

**Generated by:** `scripts/agent-self-wake.py` → `create_stuck_task_recovery()`

**Task template location:** Lines 425–449 in `agent-self-wake.py`

**Why STUCK TASK RECOVERY tasks themselves get stuck:**
1. **Dispatch stall:** task-watcher not running or not claiming tasks
2. **Redispatch bounce:** Task redistributed 3+ times (MAX_REDISPATCH_COUNT) → excluded from load balancing
3. **Domain incompatibility:** Task domain doesn't match idle agents' domains
4. **Credential failure:** Agent has invalid API credentials

**Resolution procedure:**

```bash
# 1. Check if task-watcher is dispatching
tail -20 ~/.openclaw/agents/main/logs/task-watcher-dispatch.json

# 2. Check for stalled agents with recovery tasks
grep -l "STUCK TASK RECOVERY" ~/.openclaw/agents/*/tasks/*.md 2>/dev/null

# 3. Check if the agents have valid credentials
for agent in temujin mongke chagatai jochi ogedei; do
    echo "=== $agent ==="
    grep ANTHROPIC_AUTH_TOKEN ~/.openclaw/agents/$agent/.claude/settings.json 2>/dev/null | head -c 60
    echo
done

# 4. Check redispatch count (if >= 3, task won't redistribute)
grep -l "redispatch_count: 3" ~/.openclaw/agents/*/tasks/*.md 2>/dev/null | wc -l

# 5. Force-start a stuck agent (if task-watcher is healthy but agent isn't being picked)
# Move a high-priority task to an idle agent manually:
mv ~/.openclaw/agents/jochi/tasks/high-*.md ~/.openclaw/agents/chagatai/tasks/ 2>/dev/null
# Then update Neo4j ownership:
python3 -c "
from neo4j_task_tracker import update_task_agent
update_task_agent('TASK_ID_HERE', 'chagatai')
"
```

**Quick fix for dispatch stall:**
If task-watcher isn't running or isn't dispatching:
```bash
# Check task-watcher status
pgrep -f "task-watcher" && echo "Running" || echo "STOPPED"

# If stopped, restart
python3 ~/.openclaw/agents/main/scripts/task-watcher.py --daemon

# If running but not dispatching, check for lock file
rm ~/.openclaw/agents/main/logs/task-watcher.lock 2>/dev/null
```

**Quick fix for redispatch bounce:**
Tasks with `redispatch_count >= 3` are excluded from load balancing. To re-enable:
```bash
# Find bounced tasks
grep -l "redispatch_count: 3" ~/.openclaw/agents/*/tasks/*.md 2>/dev/null

# Reset redispatch count for a specific task
sed -i '' 's/redispatch_count: 3/redispatch_count: 0/' PATH_TO_TASK.md
```

**Prevention:** Monitor `logs/task-watcher-dispatch.json` for consecutive "no tasks dispatched" entries. If this exceeds 3 cycles (15 min), investigate dispatch pipeline.
...
- Version: 1.1
- Changes: Added STUCK TASK RECOVERY troubleshooting section (dispatch stall, redispatch bounce, credential failure)
