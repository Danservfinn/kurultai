...
|------|---------|------------|----------|
| **Personal** | File-based (per-agent) | Kublai only | Your preferences, personal history, friend names |
| **Operational** | Neo4j Shared Pool | All 6 agents | Research findings, code patterns, process insights |

**Kublai acts as privacy firewall**: When delegating, Kublai strips personal context before other agents see requests. Results return through Kublai for re-personalization.
...
### What's Being Kept (File-Based)

- **SOUL.md** - Persona boundaries (loaded at session start)
- **USER.md** - User identity
- **AGENTS.md** - Operating instructions
- **TOOLS.md** - Tool reference
- **MEMORY.md** - Personal context (Kublai only, never shared)
- **memory/YYYY-MM-DD.md** - Daily working notes (per-agent)
- **IDENTITY.md** - Agent self-identity (filled by each agent)
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  PERSONAL (File-based)        OPERATIONAL (Neo4j)               │
│  ─────────────────────        ───────────────────               │
│  Kublai's workspace only      Shared across all agents          │
│                                                                  │
│  SOUL.md ─────────────────┐   ┌─────────────────────────┐       │
│  USER.md                  │   │  GRAPH DATABASE         │       │
│  AGENTS.md                │   │                         │       │
│  TOOLS.md                 │   │  Research, Content      │       │
│  MEMORY.md ←──────────────┤   │  Applications, Analysis │       │
│  memory/*.md              │   │  Concepts, Tasks        │       │
│  IDENTITY.md              │   │  Synthesis, Insights    │       │
│                           │   │                         │       │
│  (Query for personal      │   │  (Query for operational │       │
│   context only)           │   │   knowledge)            │       │
│                           │   └─────────────────────────┘       │
│  HEARTBEAT.md ────────────┘                                     │
│  (populated from Neo4j)                                         │
│                                                                  │
│  Kublai filters: Personal stays private,                        │
│  operational goes to Neo4j                                       │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
| **Ögedei** | ops | Operations Manager | Process management, documentation, deadlines |
...
// Agent-to-agent collaboration (query-time, not subscription-based)
(a1:Agent)-[:LEARNED {
  knowledge_id: UUID,
  knowledge_type: string,
  timestamp: datetime
...
// Direct collaboration relationships (semantic, not routing)
(a1:Agent)-[:COLLABORATES_WITH {
  domain: string,
  frequency: float
}]->(a2:Agent)
...
### OpenClaw Native Broadcast (Message Routing)

All messages route through OpenClaw's native broadcast groups. This is configured in `moltbot.json`:
...
All agents receive all Signal messages in parallel. Each agent decides whether to respond based on:
1. **Relevance** - Does this match my role/capabilities?
2. **Delegation** - Did Kublai explicitly delegate this to me?
3. **Interest** - Query Neo4j for related knowledge I should contribute

### Query-Time Filtering (Knowledge Retrieval)

Agents query Neo4j to find relevant cross-agent knowledge:

```cypher
// Möngke checking for research priorities from Jochi
MATCH (j:Agent {id: "jochi"})-[:CREATED]->(insight:Insight)
WHERE insight.category IN ["market", "competitive", "opportunity"]
  AND insight.confidence > 0.6
  AND insight.created_at > datetime() - duration('P7D')
RETURN insight.insight, insight.potential_value, insight.urgency

// Temüjin checking for security-related analysis
MATCH (j:Agent {id: "jochi"})-[:CREATED]->(analysis:Analysis)
WHERE analysis.type IN ["risk", "competitive"]
  AND "security" IN analysis.domains
  AND analysis.confidence > 0.6
RETURN analysis.findings, analysis.risks, analysis.recommendations
```

### Agent Response Strategy

| Agent | Responds When | Queries Neo4j For |
|-------|---------------|-------------------|
| **Kublai** | Always (orchestrator) | All recent activity, pending approvals |
| **Möngke** | Research tasks, insights from Jochi | Research gaps, competitive opportunities |
| **Chagatai** | Writing tasks, research completion | Source material from Möngke |
| **Temüjin** | Implementation tasks, security alerts | Technical patterns, security risks |
| **Jochi** | Analysis requests, pattern spotting | All agent outputs for trends |
| **Ögedei** | Process tasks, status updates | Workflow bottlenecks, task statuses |
